...
export default function CoachMatchs() {
  const { user } = useAuth();
  ...
  return (
    <section className="hero">
      <h1>Matchs</h1>
      <p className="lead">Posez la composition de votre équipe. Pas celle de l’adversaire. Pas la capture.</p>
      ...
      {groupes.map((g) => (
        <div key={g.code} className="journee-block">
          ...
          {g.items.map((m) => (
            m.statut === "programme" ? (
              <AVenirLigne key={m.id} match={m} clubsById={clubsById} to={`/coach/matchs/${m.id}`} />
            ) : (
              <Link key={m.id} to={`/coach/matchs/${m.id}`} className="avenir-row termine-row">
